...
<SideFigure alt="Vim modal editing diagram" caption="A typical Vim normal-mode keymap." />

At the heart of Vim's philosophy is the idea of minimizing keystrokes to achieve maximum efficiency.

<PullQuote>
    Vim's keyboard-centric design allows developers to perform virtually all
    coding tasks without leaving the home row.
</PullQuote>
...
<MarginNote>
    Vimscript is the original scripting language; modern setups often replace it
    with Lua via Neovim.
</MarginNote>
...
<SideFigure
    alt="Terminal with Vim over SSH"
    caption="Vim running over SSH on a remote server."
    width={400}
    height={260}
/>
